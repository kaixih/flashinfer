...
if [[ -z "${FORCE_TACTIC:-}" ]]; then
  echo "FORCE_TACTIC must be set as tile_N,moe_tactic, e.g. FORCE_TACTIC=8,217" >&2
  exit 2
fi
...
SAFE_TACTIC=${FORCE_TACTIC/,/_}
RUN_ID=${RUN_ID:-sglang-pure-piecewise-maxbs${CUDA_GRAPH_MAX_BS}-bf16-force-${SAFE_TACTIC}-$(date +%Y%m%d-%H%M%S)}
...
export FLASHINFER_FORCE_TRTLLM_BF16_MOE_TACTIC="${FORCE_TACTIC}"
...
export FLASHINFER_DEBUG_TRTLLM_BF16_VALID_CONFIGS_LIMIT="${FLASHINFER_DEBUG_TRTLLM_BF16_VALID_CONFIGS_LIMIT:-40}"
export FLASHINFER_DEBUG_TRTLLM_BF16_VALID_CONFIGS_MAX_ITEMS="${FLASHINFER_DEBUG_TRTLLM_BF16_VALID_CONFIGS_MAX_ITEMS:-400}"

echo "RUN_ID=${RUN_ID}"
echo "RUN_DIR=${RUN_DIR}"
echo "FORCE_TACTIC=${FORCE_TACTIC}"
